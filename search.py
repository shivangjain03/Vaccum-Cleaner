...
class Problem:
    ...
    def goal_test(self, state):
        ...
        if isinstance(self.goal, list):#checking if goal is a list or not and if it is a list then do following
...
class Node:
    ...
    def __repr__(self):#Provides a string representation of the node
        return "<Node {}>".format(self.state)

    def __lt__(self, node):#Compares two nodes based on their state, allowing nodes to be ordered (useful when nodes are stored in priority queues).
...
class VacuumPlanning(Problem):
    ...
    def generateSolution(self):
        ...
        if (path != None):
            self.env.set_solution(path)#sets the solution in the environment
        else:
            print("There is no solution!\n")

        if (explored != None):#explored is a list of nodes (or rooms) that the search algorithm visited while searching for the next dirty room
    ...
    def actions(self, state):
        ...
        #returns a list of actions that can be performed on that current state.

        possible_neighbors = self.env.things_near(state)#fidning neighbours
    ...
    def result(self, state, action):
        ...
        if action == 'RIGHT':#changing x-y coordinates accroding to action as state is in x-y state 
            new_state[0] += 1
        elif action == 'LEFT':
            new_state[0] -= 1
        elif action == 'UP':
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        if (self.env.costFunc == costFunctions[0]):  # basic stepCount cost
            ...
            #It should cost 3 units for each 90 degree turns
        
            cost = cost + 1 + 3*self.computeTurnCost(curNode.action, action)
            
        elif (self.env.costFunc == costFunctions[2]):  # cost function to encourage staying left of grid
            cost = cost + 1
            if (state2[0] < self.env.width // 2):#adding penalty so that it stays on left side of the grid
                cost = cost - 1
            else:
                cost = cost + 1
            ...
            cost = cost + 1
            if (state2[1] > self.env.height // 2):#adding penalty so that it stays on top side of the grid
                cost = cost - 1
            else:
                cost = cost + 1
    ...
    def computeTurnCost(self, action1, action):
        ...
        if action1 == action:#checking if the action is same as action1 
            return 0
    ...
    def findMinManhattanDist(self, pos):
        ...
        for object in self.map:
            if isinstance(object, Dirt):
                distance = distance_manhattan(pos, object.location)
    ...
    def findMinEuclidDist(self, pos):
        ...
        minDistance = 1000000
        for object in self.map:#use self.map to get the list of things in the environment
            if isinstance(object, Dirt):
                distance = distance_squared(pos, object.location)
                if distance < minDistance:
                    minDistance = distance           
        return minDistance
...
def breadth_first_graph_search(problem):
    ...
    if problem.goal_test(node.state):
        return node
    frontier = deque([node])  # Queue is used for breadth first graph search 
    explored = set()
    while True:#making it a infinite loop
        if len(frontier) == 0:
            return None, None
        
        node = frontier.popleft()#pops from the left end of the list
        explored.add(tuple(node.state))
        
        for children in node.expand(problem):#expaning it to get the children
            if tuple(children.state) not in explored and children not in frontier:
                if problem.goal_test(children.state):
                    return children, explored
...
def depth_first_graph_search(problem):
    ...
    if problem.goal_test(node.state):
        return node
    frontier = [node]  # Stack is used for Depth First Graph Search 
    explored = set()
    while True:
        if len(frontier) == 0:
            return None, None
       
        node = frontier.pop()  # Poping from the end of the list
        explored.add(tuple(node.state))
...
def best_first_graph_search(problem, f=None):
    ...
    explored = set()
    while True:
        if len(frontier) == 0:#if the frontier is empty then return None
            return None, None
        node = frontier.pop()
        if problem.goal_test(node.state):#checking if the goal is reached
            print(node)
            return node, explored
        explored.add(tuple(node.state))
        
        for children in node.expand(problem):#expanding the node to get the children
            if tuple(children.state) not in explored and children not in frontier:
                frontier.append(children)
            elif children in frontier:
                if f(children) < frontier[children]:
                    del frontier[children]
                    frontier.append(children)
